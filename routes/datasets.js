...
router.get('/update', function(req, res) {
    ...
    // Send status code for each case: -1 if error, 0 if not dataset found, and
    ...
    Dataset.findOne({write_key:apiKey}, function(err, dataset) {
        if (err) {
            ...
            // we check that the variables have already been registered
...
router.get('/request', function(req, res) {
    ...
    Dataset.findOne({readKey: apiKey}, function(err, dataset) {
        if (err) {
            console.log("Error retrieving dataset: "  + err);
            res.sendStatus(-1);
        } else if (dataset) {
            // Strip dataset from sensible information (_id and API keys)
            var cleanDataset = {ownerName: dataset.owner_name,
                                name: dataset.name,
                                index: dataset.index,
...
router.get('/:index', function(req, res) {
   ...
   Dataset.findOne({index: index}, function(err, dataset) {
       if (err) {
           ...
           // Only send non-sensible info to res (ie: stripped from API keys)
...
router.post('/', helper.authenticate, function(req, res) {
    ...
    //Delete the values from the request body so that we only talk about the
    //variables
    delete req.body.name;
    delete req.body.public;

    // This is so that we can loop through the object in reverse order
    // We do that so that the fields are saved in the right order in the db
    ...
    for (var i in propertiesList) {
        ...
        variablesFields[propertiesList[i]] = {name:req.body[propiertiesList[i]], values: Array};
    }
...
router.put('/:id/', helper.authenticate, function(req, res) {
    // Get values from the PUT request
    var name = req.body.name;
    var isPublic = req.body.public != undefined ? true : false;
    // Delete the values from the request body so we only keep information
    // about the variables
    delete req.body.name;
    delete req.body.public;
    ...
    Dataset.findById(req.params.id, function(err, dataset) {
        ...
        // If variable in request body and not in dataset, add to setList (or
        // if no variable at all in dataset
        for(var property in req.body) {
            if(!dataset.data||(req.body.hasOwnProperty(property)&!dataset.data.hasOwnProperty(property))) {
                console.log(property);
                console.log(req.body[property]);
                setList["data." + property] = {name:req.body[property], values: Array};
            }
        }
...
router.delete('/:id/', helper.authenticate, function(req, res) {
    ...
    Dataset.findById(req.params.id, function(err, dataset) {
        if (err) {
            ...
            req.session.error = "Error retrieving dataset.";
        } else {
            // Remove dataset document
            dataset.remove(function(err, dataset) {
                if (err) {
                    console.log("Error removing the dataset: " + err);
...
// POST request to update an API key
router.post('/update/key', helper.authenticate, function(req, res) {
    ...
    var updateJson = {};
    updateJson[key+"_key"] = hat(); // Generate a new API key

    // Find dataset by ID
    Dataset.findById(id, function(err, dataset) {
        if (err) {
            console.log("Error retrieving the datatset: " + err);
            req.session.error = "Error retrieving dataset. Please try again.";
            res.redirect(redirectURL);
        } else {
            // Update dataset with the new key
            dataset.update(updateJson, function(err, datasetID) {
                if (err) {
                    console.log("Error updating the dataset: " + err);
                    req.session.error = "error updating the dataset. Please try again.";
                    res.redirect(redirectURL);
                } else {
